# GoomerListaRango
API RESTful capaz de gerenciar os restaurantes e os produtos do seu cardápio.

## Introdução
APIs criadas para realizar cadastro, consulta, atualização e exclusão de restaurantes e produtos.

## Endpoint
O endpoint de conexão com as APIs é: ``` http://localhost:3333/api/v1 ```

## Recursos disponíveis
Existem os seguintes recursos listados abaixo que estão disponíveis para manipulação através dos métodos GET, POST, PUT e DELETE:
  - Restaurantes
  - Produtos
 
## Tratamento de dados
Todos os dados enviados e recebidos pela API estão/deverão ser em formato JSON (application/json).

### Restaurante
#### **GET** Listar restaurantes
<kbd>http://localhost:3333/api/v1/restaurante/listar/:cnpj</kbd>

| Parâmetro | Descrição                          |
| --------- | ---------------------------------- | 
| cnpj      | Cnpj do restaurante a ser pesquisado | 

#### HEADERS
**Content-type**
application/json

#### Example Request
``` http://localhost:3333/api/v1/restaurante/listar/85650128000155 ```

#### Example Response
<kbd> 200 - OK </kbd>
``` 
{
    "_id": "5e30dde967a18d8ee4ea1eb2",
    "cnpj": "85650128000155",
    "nome": "Restaurante X",
    "endereco": "Rua restaurante, 15",
    "horarioFuncionamento": [
        {
            "_id": "5e30dde967a18d8ee4ea1eb4",
            "diaSemana": "Terça",
            "horarioAberto": "18:15",
            "horarioFechado": "18:30"
        },
        {
            "_id": "5e30dde967a18d8ee4ea1eb3",
            "diaSemana": "Quarta",
            "horarioAberto": "18:15",
            "horarioFechado": "21:30"
        }
    ],
    "fotoRestaurante": "https://nodestoreteste.blob.core.windows.net/restaurant-images/28067a8c-fa2b-4417-2bdd-33fdb3f07f23.jpg",
    "created": "2020-01-29T01:20:41.215Z",
    "updated": "2020-01-29T01:20:41.215Z",
    "__v": 0
} 
```

#### **GET** Listar todos restaurantes
<kbd>http://localhost:3333/api/v1/restaurante/listarTodos</kbd>

#### HEADERS
**Content-type**
application/json

#### Example Request
``` http://localhost:3333/api/v1/restaurante/listarTodos ```

#### Example Response
<kbd> 200 - OK </kbd>
```
[
    {
        "_id": "5e30c840dfaa834be4b1abec",
        "cnpj": "27660128000138",
        "nome": "Teste teste",
        "endereco": "Rua teste, 2",
        "horarioFuncionamento": [
            {
                "_id": "5e30c840dfaa834be4b1abed",
                "diaSemana": "Segunda",
                "horarioAberto": "18:15",
                "horarioFechado": "18:30"
            }
        ],
        "fotoRestaurante": "https://nodestoreteste.blob.core.windows.net/restaurant-images/140c353b-082d-fb1a-a66a-16c6d4eb5ec0.jpg",
        "created": "2020-01-28T23:48:16.206Z",
        "updated": "2020-01-28T23:48:16.206Z",
        "__v": 0
    },
    {
        "_id": "5e30c887dfaa834be4b1abee",
        "cnpj": "27990128000103",
        "nome": "Restaurante X",
        "endereco": "Rua restaurante, 15",
        "horarioFuncionamento": [
            {
                "_id": "5e30df4a67a18d8ee4ea1ed6",
                "diaSemana": "Terça",
                "horarioAberto": "18:15",
                "horarioFechado": "21:30"
            },
            {
                "_id": "5e30df4a67a18d8ee4ea1ed5",
                "diaSemana": "Quarta",
                "horarioAberto": "18:15",
                "horarioFechado": "21:30"
            }
        ],
        "fotoRestaurante": "https://nodestoreteste.blob.core.windows.net/restaurant-images/602ec99e-f2aa-8b43-8371-4a277dabe334.jpg",
        "created": "2020-01-28T23:50:38.862Z",
        "updated": "2020-01-29T01:26:34.293Z",
        "__v": 0
    },
] 
```

#### **POST** Cadastrar restaurantes
<kbd>http://localhost:3333/api/v1/restaurante/cadastrar</kbd>

#### HEADERS
**Content-type**
application/json

#### Example Request
``` http://localhost:3333/api/v1/restaurante/cadastrar ```

#### Body (JSON)
```
{
  "cnpj": "85650128000158",
  "nome": "Restaurante X",
  "endereco": "Rua restaurante, 15",
  "horarioFuncionamento": [
    {
      "diaSemana": "Terça",
      "horarioAberto": "18:15",
      "horarioFechado": "18:30"
    },
    {
      "diaSemana": "Quarta",
      "horarioAberto": "18:15",
      "horarioFechado": "21:30"
    }
  ],
  "fotoRestaurante": "data:image/jpeg;base64,"
}
```

> O campo fotoRestaurante deve ser uma imagem com o endereço em base 64.

#### Example Response
<kbd> 200 - OK </kbd>
``` 
{
    "_id": "5e30dde967a18d8ee4ea1eb2",
    "cnpj": "85650128000155",
    "nome": "Restaurante X",
    "endereco": "Rua restaurante, 15",
    "horarioFuncionamento": [
        {
            "_id": "5e30dde967a18d8ee4ea1eb4",
            "diaSemana": "Terça",
            "horarioAberto": "18:15",
            "horarioFechado": "18:30"
        },
        {
            "_id": "5e30dde967a18d8ee4ea1eb3",
            "diaSemana": "Quarta",
            "horarioAberto": "18:15",
            "horarioFechado": "21:30"
        }
    ],
    "fotoRestaurante": "https://nodestoreteste.blob.core.windows.net/restaurant-images/28067a8c-fa2b-4417-2bdd-33fdb3f07f23.jpg",
    "created": "2020-01-29T01:20:41.215Z",
    "updated": "2020-01-29T01:20:41.215Z",
    "__v": 0
} 
```

#### **PUT** Atualizar restaurantes
<kbd>http://localhost:3333/api/v1/restaurante/atualizar</kbd>

#### HEADERS
**Content-type**
application/json

#### Example Request
``` http://localhost:3333/api/v1/restaurante/atualizar ```

#### Body (JSON)
```
{
  "cnpj": "85650128000158",
  "nome": "Restaurante X",
  "endereco": "Rua restaurante, 15",
  "horarioFuncionamento": [
    {
      "diaSemana": "Terça",
      "horarioAberto": "18:15",
      "horarioFechado": "23:30"
    },
    {
      "diaSemana": "Quarta",
      "horarioAberto": "18:15",
      "horarioFechado": "21:30"
    }
  ],
  "fotoRestaurante": "https://nodestoreteste.blob.core.windows.net/restaurant-images/28067a8c-fa2b-4417-2bdd-33fdb3f07f23.jpg"
}
```

> A busca será feita pelo cnpj, esse dado será recebido pelo Body(JSON).

#### Example Response
<kbd> 200 - OK </kbd>
```
{
    "_id": "5e30dde967a18d8ee4ea1eb2",
    "cnpj": "85650128000155",
    "nome": "Restaurante X",
    "endereco": "Rua restaurante, 15",
    "horarioFuncionamento": [
        {
            "_id": "5e30dde967a18d8ee4ea1eb4",
            "diaSemana": "Terça",
            "horarioAberto": "18:15",
            "horarioFechado": "23:30"
        },
        {
            "_id": "5e30dde967a18d8ee4ea1eb3",
            "diaSemana": "Quarta",
            "horarioAberto": "18:15",
            "horarioFechado": "21:30"
        }
    ],
    "fotoRestaurante": "https://nodestoreteste.blob.core.windows.net/restaurant-images/28067a8c-fa2b-4417-2bdd-33fdb3f07f23.jpg",
    "created": "2020-01-29T01:20:41.215Z",
    "updated": "2020-01-29T01:20:41.215Z",
    "__v": 0
} 
```

#### **DELETE** Excluir restaurante
<kbd>http://localhost:3333/api/v1/restaurante/excluir</kbd>

#### HEADERS
**Content-type**
application/json

#### Example Request
``` http://localhost:3333/api/v1/restaurante/excluir ```

#### Body (JSON)
```
{
  "cnpj": "85650128000158",
  "nome": "Restaurante X",
  "endereco": "Rua restaurante, 15",
  "horarioFuncionamento": [
    {
      "diaSemana": "Terça",
      "horarioAberto": "18:15",
      "horarioFechado": "23:30"
    },
    {
      "diaSemana": "Quarta",
      "horarioAberto": "18:15",
      "horarioFechado": "21:30"
    }
  ],
  "fotoRestaurante": "https://nodestoreteste.blob.core.windows.net/restaurant-images/28067a8c-fa2b-4417-2bdd-33fdb3f07f23.jpg"
}
```

> A busca será feita pelo cnpj, esse dado será recebido pelo Body(JSON).

#### Example Response
<kbd> 200 - OK </kbd>
```
{
    "n": 0,
    "opTime": {
        "ts": "6787194482652610569",
        "t": 2
    },
    "electionId": "7fffffff0000000000000002",
    "ok": 1,
    "$clusterTime": {
        "clusterTime": "6787194482652610569",
        "signature": {
            "hash": "kefbCMSHRGBHrSEHghACUJeOK1c=",
            "keyId": "6785423384168628226"
        }
    },
    "operationTime": "6787194482652610569",
    "deletedCount": 1
} 
```

### Produto
#### **GET** Listar produto
<kbd>http://localhost:3333/api/v1/restaurante/listar/:id</kbd>

| Parâmetro | Descrição                          |
| --------- | ---------------------------------- | 
| id        | ObjectId gerado ao criar produto   | 

#### HEADERS
**Content-type**
application/json

#### Example Request
``` http://localhost:3333/api/v1/produto/listar/5e30d74167a18d8ee4ea1ea5 ```

#### Example Response
<kbd> 200 - OK </kbd>
``` 
{
    "promocao": {
        "descricao": "pastel promoção",
        "precoPromocao": 9.99,
        "horarioPromocao": [
            {
                "_id": "5e30d74167a18d8ee4ea1ea6",
                "diaSemana": "Seg",
                "horarioAberto": "18:15",
                "horarioFechado": "23:30"
            }
        ]
    },
    "_id": "5e30d74167a18d8ee4ea1ea5",
    "nome": "Pastel de Queijo",
    "preco": 12.59,
    "categoria": "Pastel",
    "fotoProduto": "https://nodestoreteste.blob.core.windows.net/product-images/c6bc6860-fe2a-bb4e-2807-c578eef9906e.jpg",
    "created": "2020-01-29T00:52:17.248Z",
    "updated": "2020-01-29T01:25:53.357Z",
    "__v": 0
} 
```

#### **GET** Listar todos produtos
<kbd>http://localhost:3333/api/v1/produto/listarTodos</kbd>

#### HEADERS
**Content-type**
application/json

#### Example Request
``` http://localhost:3333/api/v1/produto/listarTodos ```

#### Example Response
<kbd> 200 - OK </kbd>
```
[
    {
        "promocao": {
            "descricao": "black friday",
            "precoPromocao": 89.99,
            "horarioPromocao": [
                {
                    "_id": "5e2e5b8836e85e7494d9beae",
                    "diaSemana": "Seg",
                    "horarioAberto": "18:15",
                    "horarioFechado": "18:30"
                }
            ]
        },
        "_id": "5e30d6c667a18d8ee4ea1ea3",
        "nome": "Combinado 6",
        "preco": 132.59,
        "updated": "2020-01-29T00:50:14.910Z",
        "categoria": "Comida Japonesa",
        "created": "2020-01-29T00:50:14.910Z",
        "fotoProduto": "https://nodestoreteste.blob.core.windows.net/product-images/eb4ec062-3bd4-4a31-2849-694c7a1b24e4.jpg",
        "__v": 0
    },
    {
        "promocao": {
            "descricao": "pastel promoção",
            "precoPromocao": 9.99,
            "horarioPromocao": [
                {
                    "_id": "5e30d74167a18d8ee4ea1ea6",
                    "diaSemana": "Seg",
                    "horarioAberto": "18:15",
                    "horarioFechado": "23:30"
                }
            ]
        },
        "_id": "5e30d74167a18d8ee4ea1ea5",
        "nome": "Pastel de Queijo",
        "preco": 12.59,
        "categoria": "Pastel",
        "fotoProduto": "https://nodestoreteste.blob.core.windows.net/product-images/c6bc6860-fe2a-bb4e-2807-c578eef9906e.jpg",
        "created": "2020-01-29T00:52:17.248Z",
        "updated": "2020-01-29T01:25:53.357Z",
        "__v": 0
    },
] 
```

#### **POST** Cadastrar produtos
<kbd>http://localhost:3333/api/v1/produto/cadastrar</kbd>

#### HEADERS
**Content-type**
application/json

#### Example Request
``` http://localhost:3333/api/v1/produto/cadastrar ```

#### Body (JSON)
```
{
  "promocao": {
    "descricao": "pastel promoção",
    "precoPromocao": 9.99,
    "horarioPromocao": [
	    {
        	"diaSemana": "Seg",
        	"horarioAberto": "18:15",
        	"horarioFechado": "23:30"
        }
    ]
  },  
  "nome": "Pastel Pizza",
  "preco": 12.59,
  "categoria": "Pastel",
  "fotoProduto": "data:image/jpeg;base64,"
}
```

> O campo fotoProduto deve ser uma imagem com o endereço em base 64.

#### Example Response
<kbd> 200 - OK </kbd>
``` 
{
    "promocao": {
        "descricao": "pastel promoção",
        "precoPromocao": 9.99,
        "horarioPromocao": [
            {
                "_id": "5e30f82f8139eb90342133e8",
                "diaSemana": "Seg",
                "horarioAberto": "18:15",
                "horarioFechado": "23:30"
            }
        ]
    },
    "nome": "Pastel Pizza",
    "preco": 12.59,
    "categoria": "Pastel",
    "fotoProduto": "https://nodestoreteste.blob.core.windows.net/product-images/61ec93e7-42a3-3688-7469-21d92c1cbf7b.jpg",
    "created": "2020-01-29T03:12:47.436Z",
    "updated": "2020-01-29T03:12:47.436Z",
    "_id": "5e30f82f8139eb90342133e7",
    "__v": 0
}
```

#### **PUT** Atualizar produto
<kbd>http://localhost:3333/api/v1/produto/atualizar</kbd>

#### HEADERS
**Content-type**
application/json

#### Example Request
``` http://localhost:3333/api/v1/produto/atualizar ```

#### Body (JSON)
```
{
    "promocao": {
        "descricao": "pastel promoção",
        "precoPromocao": 9.99,
        "horarioPromocao": [
            {
                "_id": "5e30d74167a18d8ee4ea1ea6",
                "diaSemana": "Seg",
                "horarioAberto": "18:00",
                "horarioFechado": "23:30"
            }
        ]
    },
    "_id": "5e30df0167a18d8ee4ea1ec3",
    "nome": "Pastel de Queijo",
    "preco": 12.59,
    "categoria": "Pastel",
    "fotoProduto": "https://nodestoreteste.blob.core.windows.net/product-images/c6bc6860-fe2a-bb4e-2807-c578eef9906e.jpg",
    "created": "2020-01-29T00:52:17.248Z",
    "__v": 0
}
```

> A busca será feita pelo campo _id, esse dado será recebido pelo Body(JSON).

#### Example Response
<kbd> 200 - OK </kbd>
```
{
    "promocao": {
        "descricao": "pastel promoção",
        "precoPromocao": 9.99,
        "horarioPromocao": [
            {
                "_id": "5e30d74167a18d8ee4ea1ea6",
                "diaSemana": "Seg",
                "horarioAberto": "18:00",
                "horarioFechado": "23:30"
            }
        ]
    },
    "_id": "5e30df0167a18d8ee4ea1ec3",
    "nome": "Pastel de Queijo",
    "preco": 12.59,
    "categoria": "Pastel",
    "fotoProduto": "https://nodestoreteste.blob.core.windows.net/product-images/c6bc6860-fe2a-bb4e-2807-c578eef9906e.jpg",
    "created": "2020-01-29T00:52:17.248Z",
    "__v": 0
} 
```

#### **DELETE** Excluir produto
<kbd>http://localhost:3333/api/v1/produto/excluir</kbd>

#### HEADERS
**Content-type**
application/json

#### Example Request
``` http://localhost:3333/api/v1/produto/excluir ```

#### Body (JSON)
```
{
    "promocao": {
        "descricao": "pastel promoção",
        "precoPromocao": 9.99,
        "horarioPromocao": [
            {
                "_id": "5e30d74167a18d8ee4ea1ea6",
                "diaSemana": "Seg",
                "horarioAberto": "18:15",
                "horarioFechado": "23:30"
            }
        ]
    },
    "_id": "5e30df0167a18d8ee4ea1ec3",
    "nome": "Pastel de Queijo",
    "preco": 12.59,
    "categoria": "Pastel",
    "fotoProduto": "https://nodestoreteste.blob.core.windows.net/product-images/c6bc6860-fe2a-bb4e-2807-c578eef9906e.jpg",
    "created": "2020-01-29T00:52:17.248Z",
    "__v": 0
}
```

> A busca será feita pelo campo _id, esse dado será recebido pelo Body(JSON).

#### Example Response
<kbd> 200 - OK </kbd>
```
{
    "n": 0,
    "opTime": {
        "ts": "6787194482652610569",
        "t": 2
    },
    "electionId": "7fffffff0000000000000002",
    "ok": 1,
    "$clusterTime": {
        "clusterTime": "6787194482652610569",
        "signature": {
            "hash": "kefbCMSHRGBHrSEHghACUJeOK1c=",
            "keyId": "6785423384168628226"
        }
    },
    "operationTime": "6787194482652610569",
    "deletedCount": 1
} 
```
